...
class Tx_Phpunit_Service_TestFinderTest extends Tx_Phpunit_TestCase {
	...
	public function invalidTestCaseClassNameDataProvider() {
		...
		// Otherwise, these tests will fail with ab own PHPUnit composer installation without the phpunit-selenium package.
		if (class_exists('PHPUnit_Extensions_Selenium2TestCase', TRUE)) {
			$classNamesThatMightNotExist['extbase selenium base test class (before 6.0)'] = array('Tx_Extbase_SeleniumBaseTestCase');
			$classNamesThatMightNotExist['extbase selenium base test class (since 6.0)'] = array('TYPO3\\CMS\\Extbase\\Tests\\SeleniumBaseTestCase');
		}
