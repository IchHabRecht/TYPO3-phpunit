...
class Tx_Phpunit_ViewHelpers_ExtensionSelectorViewHelper extends Tx_Phpunit_ViewHelpers_AbstractSelectorViewHelper {
	 ...
	 */
	protected function renderForm($content) {
			// Add form fields and button to content
		$content .= $this->renderHiddenFields();
		$content .= $this->renderSubmitButton($GLOBALS['LANG']->getLL('run_all_tests'));

			// Wrap with paragraph tag
		$content = $this->renderTag('p', array(), $content);

			// Wrap with form tag and return
	...
	protected function isOptionSelected($value) {
		return ($this->userSettingService->getAsString(Tx_Phpunit_Interface_Request::PARAMETER_KEY_TESTABLE) === $value);
	}
}
