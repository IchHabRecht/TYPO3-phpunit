	...
	public function render() {
		...
		foreach ($options as $option) {
			if (strpos($option, 'selected="selected"') !== FALSE) {
				preg_match('/<option[^>]*value="([^"]*)"[^>]*>/', $option, $matches);
				$selectedExtension = $matches[1];
				break;
			}
		}
		if ($selectedExtension == Tx_Phpunit_Testable::ALL_EXTENSIONS) {
			$selectedExtension = '';
		}
		...
		$content = $this->renderTag(
			...
			array(
				...
				'style' => ($selectedExtension !== '') ? $this->createIconStyle($selectedExtension, FALSE) : ''
			),
			implode('', $options)
		);
