...
abstract class Tx_Phpunit_ViewHelpers_AbstractSelectorViewHelper extends Tx_Phpunit_ViewHelpers_AbstractViewHelper {
	 ...
	 *        the key of a loaded extension, may also be "typo3", must not be empty
	 * @param boolean $hsc either use htmlspecialchars function
	 *
	 ...
	 */
	protected function createIconStyle($extensionKey, $hsc = TRUE) {
		if ($extensionKey === '') {
		...
		$iconPath = $testable->getIconPath();
		if ($hsc === TRUE) {
			$iconPath = htmlspecialchars($iconPath);
		}

		return 'background: url(' . $iconPath . ') 3px 50% white no-repeat; padding: 1px 1px 1px 24px;';
	}

	/**
	 * Gets all options rendered as an array.
	 *
	 * @return array<string> all options as html string provided in an indexed array, might be empty
	 */
	abstract protected function getOptions();

	/**
	 * Renders any html tag with it's own parameter either around some content.
	 * If the content is empty, the tag gets closed properly.
	 *
	 ...
	 *
	 * @return string html string either surrounds content or as single tag
	 *
	 * @throws InvalidArgumentException if the given tagName is empty
	 */
	protected function renderTag($tagName, $parameters = array(), $content = '') {
		if ($tagName === '') {
