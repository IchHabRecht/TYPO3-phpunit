...
class Tx_Phpunit_ViewHelpers_Jasmine_Be_ConfigurationViewHelper extends Tx_Fluid_ViewHelpers_Be_AbstractBackendViewHelper {
	...
	public function render($extKey = NULL, $loadHtmlSpecRunner = FALSE) {
		...
		if (!empty($extKey) && $loadHtmlSpecRunner === TRUE) {
			$this->pageRenderer->addExtOnReadyCode('TYPO3.Phpunit.Jasmine.App.loadHtmlSpecRunner("' . $extKey . '")');
		}

			// Load ExtDirect code
		$this->pageRenderer->addJsFile($this->backPath . TYPO3_mainDir . 'ajax.php?ajaxID=ExtDirect::getAPI&namespace=TYPO3.Phpunit', NULL, FALSE, TRUE);
