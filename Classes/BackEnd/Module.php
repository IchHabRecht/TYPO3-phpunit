...
class Tx_Phpunit_BackEnd_Module extends t3lib_SCbase {
	...
	protected function createTestSuiteWithTestCasesToBeRun() {
		...
		if ($this->request->hasString(Tx_Phpunit_Interface_Request::PARAMETER_KEY_TESTCASE)) {
			...
			list($selectedTestCaseName, $unused) = explode('::', $this->request->getAsString(Tx_Phpunit_Interface_Request::PARAMETER_KEY_TEST));
		}

		if ($selectedTestCaseName !== NULL) {
			foreach (get_declared_classes() as $className) {
				if (($className === $selectedTestCaseName) && $this->testFinder->isValidTestCaseClassName($className)) {
					$testSuite->addTestSuite($className);
					break;
				}
			}
	...
	protected function runSingleTest(
		...
		// @todo Why is $testIdentifier unused? Is this a bug or can it be removed?
