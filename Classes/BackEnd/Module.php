...
class Tx_Phpunit_BackEnd_Module extends t3lib_SCbase {
	 ...
	 * GET/POST configuration (i.e. selected testcase/test).
	 *
	...
	protected function createTestSuiteWithTestCasesToBeRun() {
		...
		// In case a single testcase is selected, we only add that testcase to the testsuite.
		// If a single test is selected, we only add the testcase which contains that test.
		...
		if($this->request->hasString(Tx_Phpunit_Interface_Request::PARAMETER_KEY_TESTCASE)) {
			...
			list($class, $method) = explode('::', $this->request->getAsString(Tx_Phpunit_Interface_Request::PARAMETER_KEY_TEST));
			$selectedTestCase = $class;
		}

		if($selectedTestCase !== NULL) {
			foreach (get_declared_classes() as $class) {
				if($this->shouldAddToTestSuite($class) && $class === $selectedTestCase) {
					$testSuite->addTestSuite($class);
					return $testSuite;
				}
			}
		...
		// If neither a single testcase or a test is selected, all known testcases are added
		foreach (get_declared_classes() as $class) {
			if ($this->shouldAddToTestSuite($class)) {
				$testSuite->addTestSuite($class);
			}
		}
	...
	/**
	 * Determine if the passed test class should be added to the TestSuite. This is
	 * determined by the class hierarchy and the phpunit extension's configuration
	 * (e.g. whether or not Selenium test cases should be included).
	 *
	 * @author	Lucas Jenss <lucas@gosign.de>
	 * @param	string	Class name to test
	 * @return	boolean	TRUE if the class should be added, otherwise FALSE
	 */
	protected function shouldAddToTestSuite($class) {
		$classReflection = new ReflectionClass($class);
