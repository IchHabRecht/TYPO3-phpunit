...
class Tx_Phpunit_Service_TestFinder implements t3lib_Singleton {
	...
	protected function createTestableForSingleExtension($extensionKey) {
		...
		$possibleIconArray = array('ext_icon.gif', 'ext_icon.png');
		foreach ($possibleIconArray as $iconFile) {
			if (file_exists(t3lib_extMgm::extPath($extensionKey) . $iconFile)) {
				$testable->setIconPath(t3lib_extMgm::extRelPath($extensionKey) . $iconFile);
				break;
			}
		}
